...
public class SlimDimontTool implements JstacsTool { 
	...
	public ToolParameterSet getToolParameters() {
		...
		parameters.add(new FileParameter( "Background file", "The file name of the file containing background sequences in format", "fasta,fa,fas",false ) );
	...
	public ToolResult run(ToolParameterSet parameters, Protocol protocol, ProgressUpdater progress, int threads) throws Exception {
		...
		DataSet bgData = null;
		if(parameters.getParameterAt(1).hasDefaultOrIsSet()){
		...
		String position = parameters.getParameterAt(2).getValue().toString();
		String value = parameters.getParameterAt(3).getValue().toString();
		double sd = (Double) parameters.getParameterAt(4).getValue();
		String weightingFactor = parameters.getParameterAt(5).getValue().toString();
		int restarts = (Integer) parameters.getParameterAt(6).getValue();
		int motifLength = (Integer) parameters.getParameterAt(7).getValue();
		int modelType = (Integer) ((SelectionParameter)parameters.getParameterAt(8)).getSelected();
		int ordDist =  (int) ((ParameterSet)((SelectionParameter)parameters.getParameterAt(8)).getValue()).getParameterAt(0).getValue();
		int bgOrder = (Integer) parameters.getParameterAt(9).getValue();
		double ess = (Double) parameters.getParameterAt(10).getValue();
		boolean delete = (Boolean) parameters.getParameterAt(11).getValue();
		boolean modify = (Boolean) parameters.getParameterAt(12).getValue();
	...
	*//**
	...
	//	AlphabetContainer con = DNAAlphabetContainer.SINGLETON;
		...
		for(int m=0,n=0;m<best.length;m++){
			if(use[m]){
				...
				result.add(new TextResult("SlimDimont "+(n+1), "The SlimDimont classifier", new FileParameter.FileRepresentation("", storables[index[m]].toXML().toString()), "xml", "SlimDimont", null, true));
	...
	public static TextResult getTextResult( DataSet data, double[] weights, Pair<double[][][], int[][]> pair, int motifLength, int motifIndex, DifferentiableStatisticalModel model, LinkedList<Sequence> bs, DoubleList bsWeights, String valueKey ) throws Exception {
		
		SplitSequenceAnnotationParser pars = new SplitSequenceAnnotationParser( ":", ";" );
		
		
		StringBuffer sb = new StringBuffer();
		sb.append("Sequence index\tPosition\tStrand\tp-value\t-log10(p-value)\tScore\tBinding site\tAdjusted binding site\tSignal\tSequence annotation\n");
		...
		for(int i=0;i<data.getNumberOfElements();i++){//TODO Jens->Jan: pos.length vs. data.getNumberOfElements()
			for(int j=0;j<pos[i].length;j++){
				...
				sb.append((i+1)+"\t"+(curr+1)+"\t"+(rc ? "-" : "+")+"\t"+pvals[i][j]+"\t"+(-Math.log10( pvals[i][j] ))+"\t"+score+"\t"+sub.toString()+"\t"+sub2.toString()+"\t"+data.getElementAt( i ).getSequenceAnnotationByType( valueKey, 0 ).getIdentifier()+"\t"+pars.parseAnnotationToComment( ' ', data.getElementAt( i ).getAnnotation() ).substring( 1 )+"\n");
		...
		TextResult tr = new TextResult("Predictions for motif "+(motifIndex+1), "", new FileParameter.FileRepresentation("", sb.toString()),"tsv","SlimDimont",null,true);
		return tr;
	}
...
//			}//TODO end
	...
	public String getHelpText() {
		try {
			return FileManager.readInputStream( SlimDimontTool.class.getClassLoader().getResourceAsStream( "projects/slim/help.txt" ) ).toString();
		} catch ( IOException e ) {
			e.printStackTrace();
			return "";
		}
	...
	public ResultEntry[] getDefaultResultInfos() {
		return new ResultEntry[]{
				new ResultEntry(TextResult.class, "xml", "SlimDimont 1"),
	...
	public ToolResult[] getTestCases(String path) {
		try {
			return new ToolResult[]{
					new ToolResult(FileManager.readFile(path+File.separator+"xml/slimdimont.xml"))};
		} catch( Exception e ) {
			e.printStackTrace();
			return null;
		}
