...
public class RegionPWMScan2 {

	
	public static void main(String[] args) throws Exception {
		
		ArrayList<SimpleEntry<String, double[][]>> pwms = PFMComparator.readPFMsFromJasparFastA( new BufferedReader( new FileReader(args[0]) ) );
		
		TrainableStatisticalModel model = new PFMWrapperTrainSM(DNAAlphabetContainer.SINGLETON,null,pwms.get(0).getValue(),0.0);
		
		StrandTrainSM model2 = new StrandTrainSM(model, 1, 0.5, 1, new IterationCondition(1), Parameterization.THETA);
		...
		while( (seqs = readNextSequences(read, lastHeader)) != null ){
			...
			for(int i=0;i<ds.getNumberOfElements();i++){
				...
				for(int j=0;j<seq.getLength()-model2.getLength()+1;j++){
					double[] compScore = new double[]{model2.getLogProbFor(0, seq,j,j+model2.getLength()-1),
							model2.getLogProbFor(1, seq,j,j+model2.getLength()-1)};
