...
public class DimontTool implements JstacsTool {
	...
	public ToolParameterSet getToolParameters() {
		...
		parameters.add( new EnumParameter( HTS_SequenceWeights.class , "Sequence weighting mode", true ) );
		parameters.add( new EnumParameter( HTS_KmerRanking.class , "Kmer ranking mode", true ) );
		parameters.add( new EnumParameter( HTS_KmerFilter.class , "Kmer filtering mode", true ) );
		parameters.add( new EnumParameter( HTS_InitialPwms.class , "PWM initializing mode", true ) );
		
		parameters.add( new SimpleParameter( DataType.DOUBLE, "Motif shortening threshold", "Entropy threshold for motif shortening", true, new NumberValidator<Double>( 0.2, 0.5 ), 0.2 ) );
		parameters.add( new SimpleParameter( DataType.BOOLEAN, "Multiple position extension", "Allow multiple position extension", true, false ) );
		parameters.add( new SimpleParameter( DataType.INT, "Within motif gaps", "The number of gaps allowed during motif extension", true, new NumberValidator<Integer>(0,5), 0 ) );
	...
	public ToolResult run(ToolParameterSet parameters, Protocol protocol, ProgressUpdater progress, int threads) throws Exception {
		...
		HTS_SequenceWeights sequenceWeightingMode = (HTS_SequenceWeights) parameters.getParameterAt(11).getValue();
		HTS_KmerRanking kmerRankingMode = (HTS_KmerRanking) parameters.getParameterAt(12).getValue();
		HTS_KmerFilter kmerFilteringMode = (HTS_KmerFilter) parameters.getParameterAt(13).getValue();
		HTS_InitialPwms PwmInitializingMode = (HTS_InitialPwms) parameters.getParameterAt(14).getValue();
		double shortThreshold = (Double) parameters.getParameterAt(15).getValue();
		boolean multiplePosExt = (Boolean) parameters.getParameterAt(16).getValue();
		int gaps = (Integer) parameters.getParameterAt(17).getValue();
		...
		weights[0] = HTS_SequenceWeights.compute( data[0], raw, wf, sequenceWeightingMode,protocolString );
		////////////////////////////////////////////
		...
		////////////////////////////////////////////
		// Marie: alternative small data set for Marie_InitialPwms.CYCLE_CORRECTION
		Pair<DataSet,double[][]> alternativeSmallData = null;
		double pwmCorrectionFactor = 1;
		if (PwmInitializingMode == HTS_InitialPwms.CYCLE_CORRECTION) {
			int minCycle = (int) ToolBox.min(raw);
			int maxCycle = (int) ToolBox.max(raw);
			
			pwmCorrectionFactor = HTS_SequenceWeights.findPwmCorrectionFactor(minCycle, maxCycle, completeData, raw);
			protocolString[0] = protocolString[0] + "Pwm correction factor: "+pwmCorrectionFactor+"\n";
			
			ArrayList<Sequence> sequences = new ArrayList<Sequence>();
			ArrayList<Double> classes = new ArrayList<Double>();
			for (int n=0;n<raw.length;n++) {
				if (raw[n]==minCycle) {
					sequences.add(completeData.getElementAt( n ));
					classes.add(0.0);
				}
				else if(raw[n]==maxCycle) {
					sequences.add(completeData.getElementAt( n ));
					classes.add(1.0);
				}
			}
			double[] w = new double[classes.size()];
			for (int i=0;i<classes.size();i++) {
				w[i] = classes.get(i);
			}
			
			alternativeSmallData = getSmallDataSets( new double[][]{w, Interpolation.getBgWeight( w )} , sequences.toArray( new Sequence[sequences.size()] ), 0.3 , 1000 );
			
		}
		protocol.append("-----------------------------------------\n");
		protocol.append(protocolString[0]+"\n");
		////////////////////////////////////////////
		...
		if( percentKmers > 0.0 ) {		
			...
			////////////////////////////////////////////
			ComparableElement<String, Double>[] kmersWithWeights = HTS_KmerRanking.compute( k, smallData, smallWeight[0], kmerRankingMode );
			ComparableElement<String, Double>[] array = HTS_KmerFilter.apply(Math.max( 50, (int)Math.ceil( percentKmers*restarts )), kmersWithWeights, kmerFilteringMode);
			////////////////////////////////////////////
			...
			for( int z = 0; z < array.length; z++ ) {
				...
				HTS_InitialPwms.compute((AbstractSingleMotifChIPper)score[0], motifLength, Sequence.create(con, array[z].getElement()), smallData, smallWeight[0], alternativeSmallData, pwmCorrectionFactor, h, PwmInitializingMode );
		...
		for( int m = 0; m < best.length; m++ ) {
			...
			heuristic((MutableMotifDiscoverer) score[0], completeData, completeWeight, objective,mean,sds, protocol,
					shortThreshold, multiplePosExt, gaps); // Marie
	...
	private static Pair<DataSet,double[][]> getSmallDataSets(double[][] completeWeight, Sequence[] data, double percent, int maxN) throws EmptyDataSetException, WrongAlphabetException{
		...
		while(totN < maxN && currSums[0] < sums[0] && currSums[0] + completeWeight[0][ofg[idxFg-1]] <= sums[0]*percent){ // Marie: use <=
			...
			while(totN < maxN && currSums[1] < sums[1] && currSums[1] + completeWeight[1][obg[idxBg-1]] <= sums[1]*tempPerc){ // Marie: use <=
		...
		double[] rw = w.toArray();
		//////////////////////////////////////////// Marie: prevent (almost) empty data set
		if (percent < 1.0 && seqs.size() < maxN*0.05 ) {
			return getSmallDataSets(completeWeight, data, 1.0 , maxN);
		}
		////////////////////////////////////////////
	...
	private static boolean heuristic( MutableMotifDiscoverer best, DataSet completeData, double[][] completeWeight, LogGenDisMixFunction objective, double[] mean, double[] sds, Protocol protocol,
			double shortThreshold, boolean multiplePosExt, int gaps) throws Exception {
		...
		for(int im=0;im<best.getNumberOfMotifs();im++){
			...
			if (!multiplePosExt) {
				...
				while( left < kl.length &&  kl[left] < shortThreshold ){
					left++;
				}			
				while( right > -kl.length && kl[kl.length-1+right] < shortThreshold ){
					right--;
				}
			...
			double[][] bindingSite = smof.getPWM( im, completeData, completeWeight[0], maxExtension, maxExtension );
			//double[] entropyNew = new double[bindingSite.length];
			double[] klNew = new double[bindingSite.length];

			for( int i = 0; i < bindingSite.length; i++ ) {
				//entropyNew[i] = 0;
				klNew[i] = 0;
				for( int j = 0; j < bindingSite[i].length; j++ ) {
					if( bindingSite[i][j] > 0 ) {
						//entropyNew[i] -= bindingSite[i][j] * Math.log( bindingSite[i][j])/log2;
						klNew[i] += bindingSite[i][j] * Math.log( bindingSite[i][j]/bgDistr[j] );
					}
				}
			}
			
			// remove position
			while( left < pwm.length &&  klNew[maxExtension+left] < shortThreshold ){
				left++;
			}			
			while( right > -pwm.length && klNew[maxExtension+pwm.length-1+right] < shortThreshold ){
				right--;
			}
			// add position
			
				double leftKl, rightKl;
				int allowedGaps = gaps;
				for (int i=0; i<=allowedGaps; i++) {
					if (pwm.length+right-left+i==maxMotifLength)
						break;
					leftKl = left-i>-maxExtension? klNew[maxExtension-1+left-i] : Double.MIN_VALUE;
					rightKl = right+i<maxExtension? klNew[maxExtension+pwm.length+right+i] : Double.MIN_VALUE;
					
					if (rightKl>=leftKl) {
						if (rightKl>4*thresh){
							right+= (i+1);
							if (i>0) allowedGaps=0;
							i = -1;
						}
					}
					else {
			...
			if( left == pwm.length || right == -pwm.length ) {
	...
	static int getMinimumHammingDistance(Sequence curr, Sequence seq2) throws Exception {
