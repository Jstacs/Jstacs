...
\entry{\TrainSM}{.getLogProbFor(sequence)}{Obtain the log probility (likelihood) of a sequence for a given model}
...
\entry{\GenDisMixClassifier}{}{Classifier that learns \DiffSM s using a unified learning principle}
...
\entry{new \NumericalPerformanceMeasureParameterSet}{()}{Create a set of standard performance measures that are applicable to two-class problems (binary classification)}

\entry{new \PerformanceMeasureParameterSet}{(measures)}{Create a set of performance measures}

\section{Utils}

\entry{new \Alignment}{(\AlignmentType,costs)}{Create an object for alignments of sequences}

\entry{\Alignment}{.getAlignment(seq1,seq2)}{Align to sequences}

~

\entry{\ArrayHandler}{.clone(array)}{Deep clone multi-dimensional array}

\entry{\ArrayHandler}{.createArrayOf(template,num)}{Create an array containing num copies of a (cloneable) template}
