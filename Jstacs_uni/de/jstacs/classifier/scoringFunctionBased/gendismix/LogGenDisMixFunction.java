 ...
 * <br>
 * 
 * It is very important for this class that the {@link ScoringFunction#clone()} method works correctly, since each thread works on its own clones. 
 * 
 * @author Jens Keilwagen
 */
public class LogGenDisMixFunction extends SFBasedOptimizableFunction
{
	/**
	...
	{	
		...
		if( beta[LearningPrinciple.LIKELIHOOD_INDEX] != 0 )
		{
			try
			{
				...
				{
					norm = Normalisation.getLogSum( norm, logClazz[counter3] + ((NormalizableScoringFunction)score[0][counter3]).getLogNormalizationConstant() );
				}
				for( counter3 = 0; counter3 < cl; counter3++ )
				{
					if( counter3 < shortcut[0] )
					{
						llGrad[0][counter3] -= sum[cl] * Math.exp( logClazz[counter3] + ((NormalizableScoringFunction)score[0][counter3]).getLogNormalizationConstant() - norm );
					}
					for( counter1 = shortcut[counter3]; counter1 < shortcut[counter3 + 1]; counter1++ )
					{
						llGrad[0][counter1] -= sum[cl] * Math.exp( logClazz[counter3] + ((NormalizableScoringFunction)score[0][counter3]).getLogPartialNormalizationConstant( counter1 - shortcut[counter3] ) - norm );
					}
				}
			}
			catch( Exception e )
	...
	{		
		...
		if( beta[LearningPrinciple.LIKELIHOOD_INDEX] != 0 )
		{
			for( i = 0; i < cl; i++ )
			{
				z = Normalisation.getLogSum( z, logClazz[i] + ((NormalizableScoringFunction)score[0][i]).getLogNormalizationConstant() );
			}
			ll -= (sum[cl] * z);
		}
