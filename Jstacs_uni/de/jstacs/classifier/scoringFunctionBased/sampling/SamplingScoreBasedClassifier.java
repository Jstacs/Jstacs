 ...
 * implementation of {@link de.jstacs.scoringFunctions.NormalizableScoringFunction#getSizeOfEventSpaceForRandomVariablesOfParameter(int)}. Let {@latex.inline $s_i$} be the size of the event space
 * of the random variable of parameter {@latex.inline $\\lambda_i$}, and let {@latex.inline $\\sigma^{2}$} be the class-dependent variance for the {@link SamplingScoringFunction}
...
public abstract class SamplingScoreBasedClassifier extends AbstractScoreBasedClassifier {
	...
	/**
	 * {@link SamplingScoringFunction}s
	 */
	...
	protected StringBuffer getFurtherClassifierInfos() {
		...
		ScoringFunctionSamplingComponent sfsc = getSamplingComponent();
		try{
	...
	protected void extractFurtherClassifierInfosFromXML( StringBuffer xml ) throws NonParsableException {
		...
		scoringFunctions = XMLParser.extractObjectForTags( xml, "scoringFunctions", SamplingScoringFunction[].class );
	 ...
	 * and set set of {@link SamplingScoringFunction}s for each of the classes.
	...
	protected ScoringFunctionSamplingComponent getSamplingComponent() {
		if(samplingComponent == null){
			samplingComponent = new ScoringFunctionSamplingComponent( params.getOutfilePrefix() );
		}
	...
	public void doSingleSampling( Sample[] s, double[][] weights, int numSteps, String outfilePrefix ) throws Exception{
		...
		SFBasedOptimizableFunction function = getFunction( s, weights );
		ScoringFunctionSamplingComponent sfsc = getSamplingComponent();
	 ...
	 * @author Jan Grau
	 * @see SamplingScoringFunction#getSamplingGroups(int)
	 */
	 ...
	 *
	 */
	protected class ScoringFunctionSamplingComponent implements SamplingComponent{
		...
		/**
		 * If this {@link ScoringFunctionSamplingComponent} is ready for storing sampled parameters
		 */
		...
		/**
		 * Creates a new {@link ScoringFunctionSamplingComponent} that uses temporary files
		 ...
		 * @see SamplingScoreBasedClassifier.ScoringFunctionSamplingComponent#saveParameters()
		 */
