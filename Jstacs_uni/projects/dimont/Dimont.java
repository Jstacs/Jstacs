...
public class Dimont { 
	...
	public static void main(String[] args) throws Exception {
		...
		//parse parameters
		ParameterSetTagger cParams = new ParameterSetTagger( DimontParameterSet.PREFIX, new DimontParameterSet() );
		...
		String home = cParams.getValueFromTag( DimontParameterSet.HOME, String.class );
		String fgData = home +File.separator+ cParams.getValueFromTag( DimontParameterSet.DATA, String.class );
		String infix = cParams.getValueFromTag( DimontParameterSet.INFIX, String.class );
		int motifLength = cParams.getValueFromTag( DimontParameterSet.LENGTH, Integer.class );
		int restarts = cParams.getValueFromTag( DimontParameterSet.STARTS, Integer.class );
		int fgOrder = cParams.getValueFromTag( DimontParameterSet.MOTIF_ORDER, Integer.class );
		int bgOrder = cParams.getValueFromTag( DimontParameterSet.BG_ORDER, Integer.class );
		String position = cParams.getValueFromTag( DimontParameterSet.POSITION_TAG, String.class );
		String value = cParams.getValueFromTag( DimontParameterSet.VALUE_TAG, String.class );
		double sd = cParams.getValueFromTag( DimontParameterSet.SD, Double.class );
		String weightingFactor = cParams.getValueFromTag( DimontParameterSet.WEIGHTING_FACTOR, String.class );
		double ess = cParams.getValueFromTag( DimontParameterSet.ESS, Double.class );
		boolean delete = cParams.getValueFromTag( DimontParameterSet.DELETE, Boolean.class );
		int threads;
		if( cParams.isSet(DimontParameterSet.THREADS) ) {
			threads = cParams.getValueFromTag( DimontParameterSet.THREADS, Integer.class );
		} else {
		...
		Result[][] res = run( SparseSequence.getDataSet(DNAAlphabetContainer.SINGLETON, fgData, parser),motifLength,restarts,fgOrder,bgOrder,position,value,sd,weightingFactor,ess,delete,threads, SafeOutputStream.getSafeOutputStream( System.out ));
		...
		for(int i=0;i<res.length;i++){
			...
			FileManager.writeFile( new File(home+File.separator+infix+"-pwm-"+(i+1)+".xml"), (String)res[i][2].getValue() );
			
			if(res[i].length > 3){
				ImageResult ir = (ImageResult) res[i][3];
	...
	public static Result[][] run(DataSet fgData, int motifLength, int restarts, int fgOrder, int bgOrder, String position,
			String value, double sd, String weightingFactor, double ess, boolean delete, int threads, SafeOutputStream out) throws Exception {
		...
		for( int m = 0; m < best.length; m++ ) {
			...
			heuristic((MutableMotifDiscoverer) score[0], completeData, completeWeight, objective,mean,sds, out);
	...
	private static boolean heuristic( MutableMotifDiscoverer best, DataSet completeData, double[][] completeWeight, LogGenDisMixFunction objective, double[] mean, double[] sds, SafeOutputStream out ) throws Exception {
