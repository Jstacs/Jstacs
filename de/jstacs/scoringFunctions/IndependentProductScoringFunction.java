 ...
 */
public class IndependentProductScoringFunction extends BasicIndependentProductScoringFunction implements NormalizableScoringFunction, MutableMotifDiscoverer
{
	...
	public IndependentProductScoringFunction( double ess, boolean plugIn, NormalizableScoringFunction... functions ) throws CloneNotSupportedException,
		...
		this( ess, plugIn, functions, BasicIndependentProductScoringFunction.getLengthArray( functions ) );
	}
	...
	public int getSizeOfEventSpaceForRandomVariablesOfParameter( int index ) {
		...
		return ((NormalizableScoringFunction)score[i]).getSizeOfEventSpaceForRandomVariablesOfParameter( index - params[i] );
	}
	...
	public double getLogNormalizationConstant() {
		...
		for( int i = 0; i < index.length; i++ ) {
			if( isVariable[index[i]] ) {
				...
				norm += ((NormalizableScoringFunction)score[index[i]]).getLogNormalizationConstant();
			}
		}
		return norm;
	}
		...
		for( ; i < index.length; i++ ) {
			if( index[i] == j ) {
				if( isVariable[index[i]] ) {
					...
					p = ((NormalizableScoringFunction)score[j]).getLogPartialNormalizationConstant( k );
					n = ((NormalizableScoringFunction)score[j]).getLogNormalizationConstant();
				}
				...
				if( isVariable[index[i]] ) {
					...
					normOfOther += ((NormalizableScoringFunction)score[index[i]]).getLogNormalizationConstant();
				}
			}
		}
	...
	/*
	 * (non-Javadoc)
	 * @see de.jstacs.scoringFunctions.BasicIndependentProductScoringFunction#extractFurtherInformation(java.lang.StringBuffer)
	 */
	@Override
	...
	/*
	 * (non-Javadoc)
	 * @see de.jstacs.scoringFunctions.BasicIndependentProductScoringFunction#getFurtherInformation()
	 */
	@Override
	...
	public double getLogPriorTerm() {
		double val = 0;
		for( int i = 0; i < score.length; i++ ) {
			val += ((NormalizableScoringFunction)score[i]).getLogPriorTerm();
		}
		return val;
	}
