...
/**
 * This abstract class implements the main functionality of a {@link ScoringFunction} based classifier.
 * 
...
public abstract class ScoreClassifier extends AbstractScoreBasedClassifier {
	 ...
	 * {@link ScoreClassifierParameterSet} and {@link ScoringFunction}s .
	 * 
	 * @param params
	 *            the parameter set for the classifier
	 * @param lastScore
		...
		while( i < score.length ) {
			...
			if( ( l == 0 || l == len ) && con.checkConsistency( score[i].getAlphabetContainer() ) ) {
				...
				throw new IllegalArgumentException( "Please check the length (" + l + " vs. " + len + ")" +
						" and the AlphabetContainer of the ScoringFunction with index " + i + "." );
			}
		}
	...
	protected double doOptimization( DataSet[] reduced, double[][] newWeights ) throws Exception {
		...
		int iterations = AbstractNormalizableScoringFunction.getNumberOfStarts( score );
		sostream.writeln( getInstanceName() );
		ScoringFunction[] bestSF = new ScoringFunction[score.length], secure;
		if( iterations > 1 ) {
		...
		SFBasedOptimizableFunction f = getFunction( reduced, newWeights );
	...
	protected void extractFurtherClassifierInfosFromXML( StringBuffer xml ) throws NonParsableException {
		...
		score = XMLParser.extractObjectForTags( xml, "score", ScoringFunction[].class );
	}
	 ...
	 * order.
	 * 
	 * @return the internally used {@link ScoringFunction}s in the internal
	 *         order
	 * 
	 * @throws CloneNotSupportedException
