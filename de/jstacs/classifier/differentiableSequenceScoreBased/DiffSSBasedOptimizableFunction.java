...
/**
 * This abstract class is the basis of all multi-threaded {@link OptimizableFunction}s that are based on {@link DifferentiableSequenceScore}s. 
 * 
...
public abstract class DiffSSBasedOptimizableFunction extends AbstractMultiThreadedOptimizableFunction {
	...
	/**
	 * These {@link DifferentiableSequenceScore}s are used during the parallel computation.
	 * <code>score[t]</code> contains all {@link DifferentiableSequenceScore}s that are used by thread <code>t</code>.
	 */
	...
	public DiffSSBasedOptimizableFunction( int threads, DifferentiableSequenceScore[] score, DataSet[] data, double[][] weights, LogPrior prior, boolean norm, boolean freeParams )																														throws IllegalArgumentException {
		...
		this.score = new DifferentiableSequenceScore[threads][cl];
		int i = 0, j;
		for( ; i < cl; i++ )
		{
			...
			case PLUGIN:
				...
				for( int i = 0; i < cl; i++ )
				{
					if( score[0][i] instanceof DifferentiableStatisticalModel ) {
						ess[i] = ((DifferentiableStatisticalModel)score[0][i]).getESS();
					}
					e += ess[i];
				}
				if( freeParams )
	 ...
	 */
	public abstract void reset( DifferentiableSequenceScore[] funs ) throws Exception;
